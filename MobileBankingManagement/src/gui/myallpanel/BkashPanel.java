...
public class BkashPanel extends JPanel {
    public static int totalAmountOfbKash = 100000;
    ...
    public BkashPanel(){
        ...
        lblBkashTitle = new JLabel("bKash");
        ...
        AddingButtonPanel.paymentMethodAddingButton.addActionListener(e -> {
            try {
                ...
                String query = "insert into public.bkash_transaction_info values(?,?,?,?,?)";
                PreparedStatement stmt = con.prepareStatement(query);
                stmt.setString(1, lblBkashTitle.getText());

                if (txtMobileNumber.getText().length() == 11) {
                    stmt.setString(2, txtMobileNumber.getText());
                } else {
                    JOptionPane.showMessageDialog(null, "Mobile number is sort or long","Enter valid Mobile",JOptionPane.ERROR_MESSAGE);
                }
                ...
                int requiredAmount = totalAmountOfbKash - Integer.parseInt(txtAmount.getText());
                if (amount < 50000 && amount >= 500) {
                    if(requiredAmount < 100){
                        JOptionPane.showMessageDialog(null, "Yor transaction is cross minimum account", "take 100 tk", JOptionPane.ERROR_MESSAGE);
                    }else{
                        stmt.setInt(5, Integer.parseInt(txtAmount.getText()));
                        totalAmountOfbKash = totalAmountOfbKash -amount;
                    }
                } else {
                    JOptionPane.showMessageDialog(null, "You can't transaction less 500 and big 50000 and must keep 100 your account", "Enter valid Mobile", JOptionPane.ERROR_MESSAGE);
                }
                stmt.executeUpdate();
                JOptionPane.showMessageDialog(null, " bKash Transaction Successfully");



                toMobile= BkashPanel.txtMobileNumber.getText();
                transaction_id=BkashPanel.txtTransactionID.getText();
                issue_date=BkashPanel.bKashIssueDate.getDate().toString();
                lastAmount= String.valueOf(Integer.parseInt(BkashPanel.txtAmount.getText()));
